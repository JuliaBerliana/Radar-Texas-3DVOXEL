...
class Window(QMainWindow):
    def __init__(self, parent=None, size=[], title="Applications Visualizer"):
        ...
        self.voxelLabel_index = QLabel()
        self.voxelLabel_index.setAlignment(Qt.AlignCenter)
        self.voxelLabel_index.setStyleSheet("background-color: black; border: none;")
        self.voxelLabel_index.setFixedSize(384, 192)

        self.voxelLabel_doppler = QLabel()
        self.voxelLabel_doppler.setAlignment(Qt.AlignCenter)
        self.voxelLabel_doppler.setStyleSheet("background-color: black; border: none;")
        self.voxelLabel_doppler.setFixedSize(384, 192)

        self.voxelLabel_snr = QLabel()
        self.voxelLabel_snr.setAlignment(Qt.AlignCenter)
        self.voxelLabel_snr.setStyleSheet("background-color: black; border: none;")
        self.voxelLabel_snr.setFixedSize(384, 192)

        self.voxelLayout.addWidget(voxelTitle)
        self.voxelLayout.addWidget(self.voxelLabel_index)
        self.voxelLayout.addWidget(self.voxelLabel_doppler)
        self.voxelLayout.addWidget(self.voxelLabel_snr)
        ...
        self.predictionLabel.setStyleSheet("font-size: 13px; font-weight: bold; color: green;")
        ...
        # Window title and menu bar initialization
    ...
    def updateVoxelGUI(self, voxel_data):
        ...
        labels = [self.voxelLabel_index, self.voxelLabel_doppler, self.voxelLabel_snr]
        channel_names = ['Index', 'Doppler', 'SNR']

        for ch in range(3):
            voxel_values = voxel_data[..., ch]
            filled = voxel_values > 0

            if np.count_nonzero(filled) == 0:
                print(f"Channel {ch} kosong.")
                labels[ch].clear()
                continue
            ...
            pixmap = QPixmap.fromImage(qimg).scaled(labels[ch].size(), Qt.KeepAspectRatio)
            labels[ch].setPixmap(pixmap)


    # def updateVoxelGUI(self, voxel_data):
    #     print(" voxel data:", voxel_data.shape)
